...
import sys
import cv
# already included in ros, no need to create manually
...
class pirate_detector:
    ...
    def pcl2_callback(self, data):
        ...
        # Using python_msg_conversions.pointclouds NOT TESTED
        xyzArr = pointclouds.pointcloud2_to_array(data)
        pirate = self.pirates.pop()
        self.pirates = []
        #Create P2P message to reach the figure
        camerapoint = PointStamped()
        camerapoint.header.frame_id = 'camera_link'
        camerapoint.header.stamp = rospy.Time().now()
        # DOES THIS FUNCTION HAS RESHAPE TO WORK LIKE THIS?
        camerapoint.position.x = xyzArr[pirate[1]][pirate[0]]['x']
        camerapoint.position.y = xyzArr[pirate[1]][pirate[0]]['y']
        camerapoint.position.z = xyzArr[pirate[1]][pirate[0]]['z']
        worldpoint = PointStamped()
        worldpoint = self.tf.transformPoint('/map', camerapoint)
        worldpose = PoseStamped()
        quaternion = quaternion_from_euler(0, 0, atan2(worldpoint.position.y-self.parent.robomap.origin[1], worldpoint.position.x-self.parent.robomap.origin[0]))
        location.header.frame_id = 'map'
        location.header.stamp = rospy.Time.now()
        location.pose.position.x = camerapoint.position.x
        location.pose.position.y = camerapoint.position.y
        location.pose.orientation.w = quaternion[3]
        location.pose.orientation.z = quaternion[2]
