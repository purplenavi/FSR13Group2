...
class TaskPlanner():
    ...
    states = ['detect_pirate', 'grab_pirate_go_home', 'drop_pirate']
    ...
    def execute(self):
        while True:
            if self.state == 'detect_pirate':
                ...
                if abs(goal.position.x - self.parent.robomap.origin[0]) < 0.05 and abs(goal.position.y - self.parent.robomap.origin[1]) < 0.05:
...
class pirate_detector:
    ...
    def get_move_goal(self):
        if self.move_goal:
            print 'returning move_goal ' + self.move_goal + '!'
        else:
    ...
    def pcl2_callback(self, data):
        print 'PCL2 Callback'
        if self.move and len(self.pirates) > 0:
    ...
    def image_callback(self, data):
        ...
        if self.UPDATE_PIRATE_DATA:
            ...
            cv.ShowImage(self.depth_window, canny)
            cv.WaitKey(1000)
