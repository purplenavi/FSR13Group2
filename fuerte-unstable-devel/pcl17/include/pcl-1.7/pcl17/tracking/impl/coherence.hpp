#ifndef PCL17_TRACKING_IMPL_COHERENCE_H_
#define PCL17_TRACKING_IMPL_COHERENCE_H_
...
namespace pcl17
{
  namespace tracking
  {
    ...
    PointCloudCoherence<PointInT>::initCompute ()
    {
      ...
      {
        PCL17_ERROR ("[pcl17::%s::compute] target_input_ is empty!\n", getClassName ().c_str ());
        return false;
      }
    ...
    PointCloudCoherence<PointInT>::compute (const PointCloudInConstPtr &cloud, const IndicesConstPtr &indices, float &w)
    {
      if (!initCompute ())
      {
        PCL17_ERROR ("[pcl17::%s::compute] Init failed.\n", getClassName ().c_str ());
        return;
      }
