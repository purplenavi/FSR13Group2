#ifndef PCL17_TRACKING_IMPL_PARTICLE_FILTER_H_
#define PCL17_TRACKING_IMPL_PARTICLE_FILTER_H_
...
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::initCompute ()
{
  ...
  if (!change_detector_)
    change_detector_ = boost::shared_ptr<pcl17::octree::OctreePointCloudChangeDetector<PointInT> >(new pcl17::octree::OctreePointCloudChangeDetector<PointInT> (change_detector_resolution_));
...
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::sampleWithReplacement
...
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::genAliasTable (std::vector<int> &a, std::vector<double> &q,
...
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::initParticles (bool reset)
{
...
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::normalizeWeight ()
{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::cropInputPointCloud (
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::calcBoundingBox (
    double &x_min, double &x_max, double &y_min, double &y_max, double &z_min, double &z_max)
{
  ...
  for (size_t i = 0; i < transed_reference_vector_.size (); i++)
  {
    ...
    PointInT Pmin, Pmax;
    pcl17::getMinMax3D (*target, Pmin, Pmax);
    if (x_min > Pmin.x)
      x_min = Pmin.x;
    if (x_max < Pmax.x)
      x_max = Pmax.x;
...
template <typename PointInT, typename StateT> bool
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::testChangeDetection
...
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::weight ()
{
  if (!use_normal_)
  {
    for (size_t i = 0; i < particles_->points.size (); i++)
...
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::computeTransformedPointCloud
...
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::computeTransformedPointCloudWithoutNormal
(const StateT& hypothesis, PointCloudIn &cloud)
{
  ...
  pcl17::transformPointCloud<PointInT> (*ref_, cloud, trans);
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::computeTransformedPointCloudWithNormal (
#ifdef PCL17_TRACKING_NORMAL_SUPPORTED
    const StateT& hypothesis, std::vector<int>& indices, PointCloudIn &cloud)
#else
...
#endif
{
  ...
  const Eigen::Affine3f trans = toEigenMatrix (hypothesis);
  // destructively assigns to cloud
  pcl17::transformPointCloudWithNormals<PointInT> (*ref_, cloud, trans);
  for ( size_t i = 0; i < cloud.points.size (); i++ )
  {
    ...
    double theta = pcl17::getAngle3D (p, n);
    if ( theta > occlusion_angle_thr_ )
      indices.push_back (i);
  }
#else
  PCL17_WARN ("[pcl17::%s::computeTransformedPointCloudWithoutNormal] use_normal_ == true is not supported in this Point Type.",
            getClassName ().c_str ());
#endif
}

template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::resample ()
{
  resampleWithReplacement ();
}

template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::resampleWithReplacement ()
{
...
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::update ()
{
...
template <typename PointInT, typename StateT> void
pcl17::tracking::ParticleFilterTracker<PointInT, StateT>::computeTracking ()
{
  ...
  // {
  //   PCL17_WARN ("too small likelihood, re-initializing...\n");
  //   initParticles (false);
  // }
}

#define PCL17_INSTANTIATE_ParticleFilterTracker(T,ST) template class PCL17_EXPORTS pcl17::tracking::ParticleFilterTracker<T,ST>;
