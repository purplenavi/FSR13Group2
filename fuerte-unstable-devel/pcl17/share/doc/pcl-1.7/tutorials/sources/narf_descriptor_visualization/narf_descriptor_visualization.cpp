...
main (int argc, char** argv)
{
  ...
  if (pcl17::console::parse (argc, argv, "-d", descriptor_size) >= 0)
  ...
  std::vector<std::vector<pcl17::Narf*> > narfs;
  ...
  for (unsigned int y=0; y<range_image.height; ++y)
  {
    for (unsigned int x=0; x<range_image.width; ++x)
    {
      ...
      pcl17::Narf::extractFromRangeImageAndAddToList (range_image, x, y, descriptor_size,
  ...
  // -----Show image-----
  // --------------------
  pcl17::visualization::RangeImageVisualizer range_image_widget ("Scene range image"),
  ...
  while (true) 
  {
    ...
    float angle_step_size = pcl17::deg2rad (360.0f)/narf.getDescriptorSize ();
    ...
    for (int descriptor_value_idx=0; descriptor_value_idx<narf.getDescriptorSize (); ++descriptor_value_idx)
    {
      ...
      //surface_patch_widget.markLine (patch_middle, patch_middle, patch_middle+line_length*sinf (angle),
                                     //patch_middle+line_length*-cosf (angle), pcl17::visualization::Vector3ub (0,255,0));
    }
    ...
    for (unsigned int i=0; i<rotations.size (); ++i)
    {
      //surface_patch_widget.markLine (radius-0.5, radius-0.5, radius-0.5f + 2.0f*radius*sinf (rotations[i]),
                                                //radius-0.5f - 2.0f*radius*cosf (rotations[i]), pcl17::visualization::Vector3ub (255,0,0));
    }
    ...
    //===================================
    const std::vector<pcl17::Narf*>& narfs_of_selected_point = narfs[selected_index];
    ...
    for (unsigned int point_index=0; point_index<range_image.points.size (); ++point_index)
    {
      ...
      std::vector<pcl17::Narf*>& narfs_of_current_point = narfs[point_index];
