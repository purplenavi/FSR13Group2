...
main (int argc, char ** argv)
{
  if (argc < 3) 
  {
    pcl17::console::print_info ("Syntax is: %s source target <options>\n", argv[0]);
    pcl17::console::print_info ("  where options are:\n");
    pcl17::console::print_info ("    -i min_sample_dist,max_dist,nr_iters ................ Compute initial alignment\n");
    pcl17::console::print_info ("    -r max_dist,rejection_thresh,tform_eps,max_iters ............. Refine alignment\n");
    pcl17::console::print_info ("    -s output.pcd ........................... Save the registered and merged clouds\n");
    pcl17::console::print_info ("Note: The inputs (source and target) must be specified without the .pcd extension\n");
  ...
  bool compute_intial_alignment = 
    pcl17::console::parse_3x_arguments (argc, argv, "-i", min_sample_dist, max_correspondence_dist, nr_iters) > 0;
  if (compute_intial_alignment)
  {
    ...
    pcl17::console::print_info ("Computed initial alignment\n");
  }

  // Refine the initial alignment
  std::string params_string;
  bool refine_alignment = pcl17::console::parse_argument (argc, argv, "-r", params_string) > 0;
  if (refine_alignment)
  {
    ...
    pcl17::console::print_info ("Refined alignment\n");
  }  

  // Transform the source point to align them with the target points
  pcl17::transformPointCloud (*src_points, *src_points, tform);

  // Save output
  std::string filename;
  bool save_output = pcl17::console::parse_argument (argc, argv, "-s", filename) > 0;
  if (save_output)
  {
    ...
    // Save the result
    pcl17::io::savePCDFile (filename, *src_points);

    pcl17::console::print_info ("Saved registered clouds as %s\n", filename.c_str ());    
  }
  ...
  else
  {
    ...
    pcl17::visualization::PointCloudColorHandlerCustom<PointT> red (src_points, 255, 0, 0);
    vis.addPointCloud (src_points, red, "src_points");

    pcl17::visualization::PointCloudColorHandlerCustom<PointT> yellow (tgt_points, 255, 255, 0);
