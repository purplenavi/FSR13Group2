...
main (int argc, char** argv)
{
  ...
  if (pcl17::console::find_argument (argc, argv, "-l") >= 0)
  {
    live_update = true;
    std::cout << "Live update is on.\n";
  }
  if (pcl17::console::parse (argc, argv, "-rx", angular_resolution_x) >= 0)
    std::cout << "Setting angular resolution in x-direction to "<<angular_resolution_x<<"deg.\n";
  if (pcl17::console::parse (argc, argv, "-ry", angular_resolution_y) >= 0)
  ...
  angular_resolution_x = pcl17::deg2rad (angular_resolution_x);
  angular_resolution_y = pcl17::deg2rad (angular_resolution_y);
  ...
  while (!viewer.wasStopped ())
  {
    ...
    if (live_update)
    {
      ...
      range_image.createFromPointCloud (point_cloud, angular_resolution_x, angular_resolution_y,
                                        pcl17::deg2rad (360.0f), pcl17::deg2rad (180.0f),
                                        scene_sensor_pose, pcl17::RangeImage::LASER_FRAME, noise_level, min_range, border_size);
      range_image_widget.showRangeImage (range_image);
    }
  }
}
