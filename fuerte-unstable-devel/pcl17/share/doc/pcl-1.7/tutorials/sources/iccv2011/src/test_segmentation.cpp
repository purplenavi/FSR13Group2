...
main (int argc, char ** argv)
{
  if (argc < 2) 
  {
    ...
    pcl17::console::print_info ("    -p dist_threshold,max_iters  ..... Subtract the dominant plane\n");
    pcl17::console::print_info ("    -c tolerance,min_size,max_size ... Cluster points\n");
    pcl17::console::print_info ("    -s output.pcd .................... Save the largest cluster\n");
    return (1);
  }
  ...
  bool subtract_plane = pcl17::console::parse_2x_arguments (argc, argv, "-p", dist_threshold, max_iters) > 0;
  if (subtract_plane)
  {
    size_t n = cloud->size ();
    cloud = findAndSubtractPlane (cloud, dist_threshold, (int)max_iters);
    pcl17::console::print_info ("Subtracted %zu points along the detected plane\n", n - cloud->size ());
  }

  // Cluster points
  double tolerance, min_size, max_size;
  std::vector<pcl17::PointIndices> cluster_indices;
  bool cluster_points = pcl17::console::parse_3x_arguments (argc, argv, "-c", tolerance, min_size, max_size) > 0;
  if (cluster_points)
  {
    clusterObjects (cloud, tolerance, (int)min_size, (int)max_size, cluster_indices);
    pcl17::console::print_info ("Found %zu clusters\n", cluster_indices.size ());
  }

  // Save output
  std::string output_filename;
  bool save_cloud = pcl17::console::parse_argument (argc, argv, "-s", output_filename) > 0;
  if (save_cloud)
  {
    ...
    if (cluster_points)
    {
      ...
      pcl17::copyPointCloud (*cloud, cluster_indices[0], *temp_cloud);
      cloud = temp_cloud;
    }
    pcl17::console::print_info ("Saving result as %s...\n", output_filename.c_str ());
  ...
  else
  {
    ...
    if (cluster_points)
    {
      for (size_t i = 0; i < cluster_indices.size (); ++i)
      {
        ...
        pcl17::PointCloud<pcl17::PointXYZ>::Ptr cluster_i (new pcl17::PointCloud<pcl17::PointXYZ>);
        pcl17::copyPointCloud (*cloud, cluster_indices[i], *cluster_i);

        // Create a random color
        pcl17::visualization::PointCloudColorHandlerRandom<pcl17::PointXYZ> random_color (cluster_i);
        ...
        vis.addPointCloud<pcl17::PointXYZ> (cluster_i, random_color, cluster_id.str ());
      }
    }
    else
    {
