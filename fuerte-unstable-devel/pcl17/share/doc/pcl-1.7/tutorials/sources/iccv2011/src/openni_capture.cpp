...
OpenNICapture::snap ()
{
  if (use_trigger_)
  {
    if (!preview_)
    {
      ...
      preview_ = boost::shared_ptr<pcl17::visualization::PCLVisualizer> (new pcl17::visualization::PCLVisualizer ());

      boost::function<void (const pcl17::visualization::KeyboardEvent&)> keyboard_cb =
...
OpenNICapture::snapAndSave (const std::string & filename)
{
  ...
  if (snapped_frame)
    pcl17::io::savePCDFile (filename, *snapped_frame);
...
void
OpenNICapture::onKeyboardEvent (const pcl17::visualization::KeyboardEvent & event)
{
