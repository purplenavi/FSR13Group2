...
loadHist (const boost::filesystem::path &path, vfh_model &vfh)
{
  ...
  pcl17::getFieldIndex (point, "vfh", fields);
                   ...
                   std::vector<vfh_model> &models)
{
  ...
  for (boost::filesystem::directory_iterator it (base_dir); it != boost::filesystem::directory_iterator (); ++it)
  {
    if (boost::filesystem::is_directory (it->status ()))
    {
      std::stringstream ss;
      ss << it->path ();
      pcl17::console::print_highlight ("Loading %s (%lu models loaded so far).\n", ss.str ().c_str (), (unsigned long)models.size ());
...
main (int argc, char** argv)
{
  if (argc < 2)
  {
    PCL17_ERROR ("Need at least two parameters! Syntax is: %s [model_directory] [options]\n", argv[0]);
    return (-1);
  }
  ...
  pcl17::console::print_highlight ("Loaded %d VFH models. Creating training data %s/%s.\n", 
  ...
  pcl17::console::print_error ("Building the kdtree index (%s) for %d elements...\n", kdtree_idx_file_name.c_str (), (int)data.rows);
